            ...
            <div className="container mx-auto px-6 -mt-20 relative z-10 pb-24">
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <motion.div
                        ...
                        className="flex-1 bg-background/80 backdrop-blur-xl rounded-[2rem] p-8 md:p-14 shadow-2xl border border-gold/10"
                    >
                        <div className="max-w-none">
                            {post.content.split('\n\n').map((paragraph, idx) => {
                                if (paragraph.trim().startsWith('###')) {
                                    return <h2 key={idx} className="text-xl font-bold text-foreground tracking-tight mt-10 mb-5 border-l-4 border-gold pl-5">{paragraph.replace('### ', '')}</h2>;
                                }
                                if (paragraph.trim().startsWith('-')) {
                                    return (
                                        <ul key={idx} className="space-y-3 my-6">
                                            {paragraph.trim().split('\n').map((item, i) => (
                                                <li key={i} className="flex items-start gap-4 text-foreground/80 text-base leading-relaxed font-normal">
                                ...
                                return (
                                    <p key={idx} className="text-foreground/80 text-base leading-relaxed mb-6 font-normal">
                                        {paragraph.trim()}
                                    </p>
                                );
                            })}
                        ...
                        <div className="mt-20 pt-10 border-t border-marble-gray/20 flex flex-col md:flex-row items-center justify-between gap-6">
                            <h4 className="text-xs font-black uppercase tracking-[0.3em] text-foreground/30">Share this Article</h4>
                            <div className="flex gap-4">
                                {[Facebook, Linkedin, Twitter].map((Icon, i) => (
                                    <button
                                        key={i}
                                        className="w-12 h-12 rounded-full border border-marble-gray/20 flex items-center justify-center text-foreground/40 hover:text-gold hover:border-gold transition-all duration-300"
                                    >
                                        <Icon size={20} />
                                    </button>
                                ))}
                    ...
                    <aside className="w-full lg:w-96 space-y-12">
                        ...
                        <div className="space-y-6">
                            <h4 className="text-xs font-black uppercase tracking-[0.3em] text-foreground/30 px-2">Related Articles</h4>
                            {relatedPosts.map((related) => (
                                <Link
                                    key={related.id}
                                    href={`/blog/${related.slug}`}
                                    className="group flex gap-4 bg-background/50 backdrop-blur-md p-4 rounded-2xl shadow-lg border border-transparent hover:border-gold transition-all"
                                >
                                    ...
                                    <div className="flex flex-col justify-center gap-1">
                                        ...
                                        <h5 className="text-sm font-bold text-foreground leading-tight group-hover:text-gold transition-colors">{related.title}</h5>
                                    </div>
                                </Link>
                            ))}
                        </div>
